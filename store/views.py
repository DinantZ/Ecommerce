...
def search(request):
    if 'keyword' in request.GET:
        ...
        if keyword:
            products = Product.objects.order_by('-created_date').filter(Q(product_name__icontains=keyword) | Q(hidden_tags__icontains=keyword))
            product_count = products.count()
    context = { 
        'products': products,
        'product_count': product_count,
...
def filter(request):
    min_price = request.GET.get('min-price')
    max_price = request.GET.get('max-price')
    if min_price is None or max_price is None or not min_price.isnumeric() or not max_price.isnumeric():
